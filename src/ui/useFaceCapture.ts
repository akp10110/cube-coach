...
export interface FaceCaptureApi {
  ...
  /** Face currently being captured; `null` once all six are done. */
...
/**
 * PR-14: drives the guided 6-face scan. Samples the live video ~5x/second,
 * classifies each of the 9 stickers, and auto-captures a face once its
 * classification holds steady for ~1s (or immediately via `captureNow`).
 ...
 */
export function useFaceCapture(active: boolean): FaceCaptureApi {
  ...
  const [centroids, setCentroids] = useState<Record<Face, HSV>>(DEFAULT_CENTROIDS)
  const [calibrated, setCalibrated] = useState(false)
  const [liveStickers, setLiveStickers] = useState<ColorMatch[] | null>(null)

  const isComplete = faceIndex >= CAPTURE_ORDER.length
  const currentFace = isComplete ? null : CAPTURE_ORDER[faceIndex]

  const finalizeFace = useCallback((samples: StickerSample[]) => {
    setStickerSamples((prev) => {
      const face = CAPTURE_ORDER[faceIndex]
      if (!face) return prev
  ...
  return { attachVideo, currentFace, isComplete, faces, liveStickers, captureNow }
}
